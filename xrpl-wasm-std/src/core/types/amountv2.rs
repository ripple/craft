...
impl Amount {
    ...
    pub const fn iou(mantissa: i64, exponent: i32, currency: CurrencyCode, account: AccountID) -> Self {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), &'static str> {
        ...
        if (amount_bits & 0x8000_0000_0000_0000) == 0 {
            ...
            let currency = CurrencyCode(currency_bytes);
...
mod tests {
    ...
    fn test_accessor_patterns() {
        let currency = CurrencyCode::from(*b"GBP");
        ...
        // Both should return the actual CurrencyCode, not wrapped in extra layers
    ...
    fn test_const_constructors() {
        ...
        const CURRENCY: CurrencyCode = CurrencyCode([0u8; 20]);
