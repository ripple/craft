...
use crate::core::type_codes::{
    ...
    STI_UINT160, STI_UINT256, STI_AMOUNT, STI_ACCOUNT
};
...
pub trait FromDataBytes: Sized {
    ...
    fn get_nested_array_element(account: &AccountID, key: &[u8], index: usize, field_key: &[u8]) -> Option<Self>;
}
...
pub trait ToDataBytes {
    ...
    fn set_nested_array_element(&self, account: &AccountID, key: &[u8], index: usize, field_key: &[u8]) -> Result<(), i32>;
}
...
#[inline]
pub fn get_data<T: FromDataBytes>(account: &AccountID, key: &(impl AsKeyBytes + ?Sized)) -> Option<T> {
    T::get_data(account, key.as_key_bytes())
}

#[inline]
pub fn get_nested_data<T: FromDataBytes>(account: &AccountID, nested: &(impl AsKeyBytes + ?Sized), key: &(impl AsKeyBytes + ?Sized)) -> Option<T> {
    T::get_nested_data(account, nested.as_key_bytes(), key.as_key_bytes())
}

#[inline]
pub fn get_array_element<T: FromDataBytes>(account: &AccountID, key: &(impl AsKeyBytes + ?Sized), index: usize) -> Option<T> {
    T::get_array_element(account, key.as_key_bytes(), index)
}

#[inline]
pub fn get_nested_array_element<T: FromDataBytes>(account: &AccountID, key: &(impl AsKeyBytes + ?Sized), index: usize, field_key: &(impl AsKeyBytes + ?Sized)) -> Option<T> {
    T::get_nested_array_element(account, key.as_key_bytes(), index, field_key.as_key_bytes())
}

#[inline]
pub fn set_data<T: ToDataBytes>(account: &AccountID, key: &(impl AsKeyBytes + ?Sized), value: T) -> Result<(), i32> {
    value.set_data(account, key.as_key_bytes())
}

#[inline]
pub fn set_nested_data<T: ToDataBytes>(account: &AccountID, nested: &(impl AsKeyBytes + ?Sized), key: &(impl AsKeyBytes + ?Sized), value: T) -> Result<(), i32> {
    value.set_nested_data(account, nested.as_key_bytes(), key.as_key_bytes())
}

#[inline]
pub fn set_array_element<T: ToDataBytes>(account: &AccountID, key: &(impl AsKeyBytes + ?Sized), index: usize, value: T) -> Result<(), i32> {
    value.set_array_element(account, key.as_key_bytes(), index)
}

#[inline]
pub fn set_nested_array_element<T: ToDataBytes>(account: &AccountID, key: &(impl AsKeyBytes + ?Sized), index: usize, field_key: &(impl AsKeyBytes + ?Sized), value: T) -> Result<(), i32> {
...
impl ToDataBytes for u32 {
    ...
    fn set_data(&self, account: &AccountID, key: &[u8]) -> Result<(), i32> {
        let be_bytes = self.to_be_bytes();
        let buffer = [STI_UINT32.into(), be_bytes[0], be_bytes[1], be_bytes[2], be_bytes[3]];
        unsafe {
            set_data_object_field(
                account.0.as_ptr(),
                account.0.len(),
    ...
    fn set_array_element(&self, account: &AccountID, key: &[u8], index: usize) -> Result<(), i32> {
        ...
        unsafe {
            set_data_array_element_field(
                account.0.as_ptr(),
                account.0.len(),
                key.as_ptr(), key.len(), index as i32,
                buffer.as_ptr(),
                9
            );
        }
    ...
    fn get_nested_data(account: &AccountID, nested: &[u8], key: &[u8]) -> Option<Self> {
        ...
        let output_len = unsafe {
            get_data_nested_object_field(
                account.0.as_ptr(),
                account.0.len(),
                nested.as_ptr(), nested.len(), key.as_ptr(), key.len(),
                buffer.as_mut_ptr(),
                21
            )
        };
