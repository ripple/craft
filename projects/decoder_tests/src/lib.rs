...
fn test_amm() {
    ...
    let _ = trace_data("  Asset currency:", &buf[.. output_len as usize], DataRepr::AsUTF8);
    ...
    let _ = trace_data("  Asset2 currency:", &buf[.. output_len as usize], DataRepr::AsUTF8);
    
    locator.repack_last(sfield::IssueIssuer);
    let output_len = unsafe {
        get_ledger_obj_nested_field(slot, locator.get_addr(), locator.num_packed_bytes(), buf.as_mut_ptr(), buf.len())
    };
    let _ = trace_data("  Asset2 issuer:", &buf[.. output_len as usize], DataRepr::AsHex);
    ...
    let _ = trace_data("  AuctionSlot Price currency:", &buf[.. output_len as usize], DataRepr::AsHex);

    locator.repack_last(sfield::IssueIssuer);
    let output_len = unsafe {
        get_ledger_obj_nested_field(slot, locator.get_addr(), locator.num_packed_bytes(), buf.as_mut_ptr(), buf.len())
    };
    let _ = trace_data("  AuctionSlot Price issuer:", &buf[.. output_len as usize], DataRepr::AsHex);
...
fn test_mpt() {
    ...
    let _ = trace_data("  MPTokenIssuanceID:", &buf[.. output_len as usize], DataRepr::AsHex);
    
    let mut value = 0u64;
    let output_len = unsafe {
        get_ledger_obj_field(slot, sfield::MPTAmount, (&mut value) as *mut u64 as *mut u8, 8)    };
