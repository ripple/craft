...
pub use xrpl_iou_value::{FLOAT_NEGATIVE_ONE, FLOAT_ONE, FLOAT_ZERO, XrplIouValue};
...
impl Number {
    ...
    pub fn to_xrpl_iou_value(&self) -> Result<XrplIouValue, NumberError> {
        ...
        // Check if mantissa fits in 54 bits
        if abs_mantissa > 0x3FFFFFFFFFFFFF {
            return Err(NumberError::InvalidArgument);
        }

        // Determine sign bit (1=positive, 0=negative)
        let sign_bit = if mantissa >= 0 { 1u64 } else { 0u64 };

        // Convert exponent: add 97 to make it unsigned
        let exponent_bits = (exponent + 97) as u8;

        // Construct the 64-bit value
        let mut value = 0u64;
        value |= 1u64 << 63; // Type bit = 1 (token)
        value |= sign_bit << 62; // Sign bit
        value |= (exponent_bits as u64) << 54; // Exponent in bits 54-61
        value |= abs_mantissa; // Mantissa in bits 0-53
...
impl MulAssign<&Number> for Number {
    fn mul_assign(&mut self, rhs: &Number) {
        ...
        if error != ffi::NumberError_NUMBER_SUCCESS {
            panic!(
                "Number multiplication failed: {:?}",
                NumberError::from(error)
            );
        }
    }
}
...
mod tests {
    ...
    fn test_xrpl_iou_value_float_negative_one_constant() {
        ...
        let number = Number::from_xrpl_iou_value(FLOAT_NEGATIVE_ONE)
            .expect("Failed to parse FLOAT_NEGATIVE_ONE");
        assert!(!number.is_zero());
        assert_eq!(number.signum(), -1);
    ...
    fn test_xrpl_iou_value_constants_arithmetic() {
        ...
        let neg_one = Number::from_xrpl_iou_value(FLOAT_NEGATIVE_ONE)
            .expect("Failed to parse FLOAT_NEGATIVE_ONE");
    ...
    fn test_xrpl_iou_value_constants_try_from() {
        ...
        let neg_one: Number = FLOAT_NEGATIVE_ONE
            .try_into()
            .expect("TryFrom FLOAT_NEGATIVE_ONE failed");
    ...
    #[test]
    fn test_to_xrpl_iou_value_zero() {
        // Test converting zero to XRPL format
        let zero = Number::new();
        let bytes = zero.to_xrpl_iou_value().expect("Failed to convert zero");
        assert_eq!(bytes, FLOAT_ZERO);

        // Verify round-trip
        let back_to_zero = Number::from_xrpl_iou_value(bytes).expect("Failed to parse back");
        assert!(back_to_zero.is_zero());
    }

    #[test]
    fn test_to_xrpl_iou_value_constants() {
        // Test converting the constant values
        let one = Number::from_xrpl_iou_value(FLOAT_ONE).expect("Failed to parse FLOAT_ONE");
        let one_bytes = one.to_xrpl_iou_value().expect("Failed to convert one");
        assert_eq!(one_bytes, FLOAT_ONE);

        let neg_one = Number::from_xrpl_iou_value(FLOAT_NEGATIVE_ONE)
            .expect("Failed to parse FLOAT_NEGATIVE_ONE");
        let neg_one_bytes = neg_one
            .to_xrpl_iou_value()
            .expect("Failed to convert negative one");
        assert_eq!(neg_one_bytes, FLOAT_NEGATIVE_ONE);
    }

    #[test]
    fn test_to_xrpl_iou_value_round_trip() {
        // Test round-trip conversion for various values
        let test_cases = vec![
            Number::from(123),
            Number::from(-456),
            Number::from(1_000_000),
            Number::from(-999_999),
        ];
        ...
        for original in test_cases {
            let bytes = original
                .to_xrpl_iou_value()
                .expect("Failed to convert to XRPL");
            let converted =
                Number::from_xrpl_iou_value(bytes).expect("Failed to convert from XRPL");

            // Due to normalization in the Number class, we compare string representations
            // or check that they're approximately equal
            let orig_str = original.to_string();
            let conv_str = converted.to_string();
            assert_eq!(orig_str, conv_str, "Round-trip failed for {}", original);
        }
    }

    #[test]
    fn test_to_xrpl_iou_value_precision() {
        // Test with high precision numbers
        let precise = Number::from_mantissa_exponent(1234567890123456, -10)
            .expect("Failed to create precise number");

        let bytes = precise
            .to_xrpl_iou_value()
            .expect("Failed to convert precise number");
        let back = Number::from_xrpl_iou_value(bytes).expect("Failed to parse back");

        // Should maintain precision through round-trip
        assert_eq!(precise.to_string(), back.to_string());
    }

    #[test]
    fn test_try_from_implementations() {
        // Test Number -> XrplIouValue TryFrom
        let number = Number::from(123);
        let bytes: XrplIouValue = (&number).try_into().expect("TryFrom reference failed");
        let bytes_owned: XrplIouValue = number.clone().try_into().expect("TryFrom owned failed");

        assert_eq!(bytes, bytes_owned);

        // Test round-trip with TryFrom
        let back: Number = bytes.try_into().expect("TryFrom back failed");
