...
use crate::sfield::{IOUCurrency, IOUIssuer, IOUValue, XRPValue};
use hex;
...
use xrpl::utils::{verify_valid_ic_value, MAX_IOU_EXPONENT, MIN_IOU_EXPONENT};
...
pub fn decode_number(s: &String) -> Option<Vec<u8>> {
    ...
    _serialize_issued_currency_value(value).ok().map(|bytes| bytes.to_vec())
}
...
fn polulate_field_names() -> HashMap<i32, String> {
    ...
    sfield_names.insert(102, "value".to_string());
    sfield_names.insert(103, "issuer".to_string());
    sfield_names.insert(104, "currency".to_string());
    sfield_names.insert(105, "sequence".to_string()); //TODO check
