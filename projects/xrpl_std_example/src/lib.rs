...
pub extern "C" fn finish() -> i32 {
    ...
    // Keep the commented out validation code from main branch
    {
        ...
        let _ = trace_data("  Account:", &account_id_clo, DataRepr::AsHex);
        
        let destination = match get_current_escrow_destination() {
            Some(v) => v,
            None => return -3,
        };
        let _ = trace_data("  Account:", &destination, DataRepr::AsHex);
    }
    {
        let keylet = [
        ...
        let _ = trace_num("wasm finish slot:", slot as i64);
        
        let mut locator = LocatorPacker::new();
        locator.pack(SignerEntries);
        let array_len = unsafe {
            host::get_ledger_obj_nested_array_len(slot, locator.get_addr(), locator.num_packed_bytes())
        };
        let _ = trace_num("wasm finish array_len:", array_len as i64);
        ...
        let _ = trace_num("wasm finish get_ledger_obj_nested_field:", nfr as i64);
