...
#define TRY_CATCH_RETURN_NULL(code) \
    try { \
        ...
        if (error) *error = handle_exception(); \
        return nullptr; \
    }
...
Number* number_new_from_int64(int64_t mantissa, NumberError* error) {
    TRY_CATCH_RETURN_NULL(
        if (error) *error = NUMBER_SUCCESS;
...
Number* number_new_from_string(const char* str, NumberError* error) {
    if (!str) {
        if (error) *error = NUMBER_ERROR_INVALID_ARGUMENT;
        return nullptr;
    }
...
NumberError number_add(Number* result, const Number* lhs, const Number* rhs) {
    if (!result || !lhs || !rhs) return NUMBER_ERROR_INVALID_ARGUMENT;
    
    TRY_CATCH_RETURN_ERROR(
        *to_cpp(result) = *to_cpp(lhs) + *to_cpp(rhs);
    )
}
...
NumberError number_negate(Number* result, const Number* num) {
    if (!result || !num) return NUMBER_ERROR_INVALID_ARGUMENT;
    
    TRY_CATCH_RETURN_ERROR(
        *to_cpp(result) = -*to_cpp(num);
    )
}
...
NumberError number_abs(Number* result, const Number* num) {
    ...
    TRY_CATCH_RETURN_ERROR(
        *to_cpp(result) = abs(*to_cpp(num));
    )
}

// In-place operations
NumberError number_add_assign(Number* lhs, const Number* rhs) {
    if (!lhs || !rhs) return NUMBER_ERROR_INVALID_ARGUMENT;
    
    TRY_CATCH_RETURN_ERROR(
        *to_cpp(lhs) += *to_cpp(rhs);
    )
}
...
NumberError number_to_int64(const Number* num, int64_t* result) {
    if (!num || !result) return NUMBER_ERROR_INVALID_ARGUMENT;
    
    TRY_CATCH_RETURN_ERROR(
        *result = static_cast<int64_t>(*to_cpp(num));
    )
}

NumberError number_to_string(const Number* num, char* buffer, size_t buffer_size) {
    if (!num || !buffer || buffer_size == 0) return NUMBER_ERROR_INVALID_ARGUMENT;
...
RoundingMode number_set_rounding_mode(RoundingMode mode) {
    return static_cast<RoundingMode>(ripple::Number::setround(static_cast<ripple::Number::rounding_mode>(mode)));
}

// Mathematical functions
NumberError number_power_uint(Number* result, const Number* base, unsigned int exponent) {
    if (!result || !base) return NUMBER_ERROR_INVALID_ARGUMENT;
    
    TRY_CATCH_RETURN_ERROR(
        *to_cpp(result) = power(*to_cpp(base), exponent);
    )
}

NumberError number_root(Number* result, const Number* value, unsigned int degree) {
    if (!result || !value) return NUMBER_ERROR_INVALID_ARGUMENT;
    
    TRY_CATCH_RETURN_ERROR(
        *to_cpp(result) = root(*to_cpp(value), degree);
    )
}
...
NumberError number_sqrt(Number* result, const Number* value) {
    ...
    TRY_CATCH_RETURN_ERROR(
        *to_cpp(result) = root2(*to_cpp(value));
    )
}
