...
use crate::host_functions_wamr::{
    ...
    // ec_add_helper, ec_mul_helper, ec_pairing_check_helper, 
    ec_add_helper_bn254, ec_mul_helper_bn254, ec_pairing_check_helper_bn254, ec_negation_helper_bn254
};
...
pub fn run_func(wasm_file: String, func_name: &str, gas_cap: Option<u32>, data_source: MockData) -> Result<bool, RuntimeError>{
    ...
    let runtime = Runtime::builder()
        ...
        // .register_host_function("ec_add_helper", ec_add_helper as *mut c_void, "(*~*~*~)i", 500, data_provider.as_ptr())
        // .register_host_function("ec_mul_helper", ec_mul_helper as *mut c_void, "(*~*~*~)i", 2000, data_provider.as_ptr())
        // .register_host_function("ec_pairing_check_helper", ec_pairing_check_helper as *mut c_void, "(*~)i", 50000, data_provider.as_ptr())
        .register_host_function("ec_add_helper_bn254", ec_add_helper_bn254 as *mut c_void, "(*~*~*~)i", 500, data_provider.as_ptr())
        .register_host_function("ec_mul_helper_bn254", ec_mul_helper_bn254 as *mut c_void, "(*~*~*~)i", 2000, data_provider.as_ptr())
        .register_host_function("ec_pairing_check_helper_bn254", ec_pairing_check_helper_bn254 as *mut c_void, "(*~)i", 10000, data_provider.as_ptr())
        .register_host_function("ec_negation_helper_bn254", ec_negation_helper_bn254 as *mut c_void, "(*~*~)i", 500, data_provider.as_ptr())
