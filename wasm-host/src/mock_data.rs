...
#[derive(Debug)]
pub enum LocatorPathItemType {
    Sfield,
    ArrayIndex,
}

#[derive(Debug)]
pub struct LocatorPathItem {
    item_type: LocatorPathItemType,
    value: i32,
}

impl LocatorPathItem {
    pub fn unpack(data: &[u8], remaining: &mut usize) -> Option<Self> {
        let item_type = match data[0] {
            0 => LocatorPathItemType::Sfield,
            1 => LocatorPathItemType::ArrayIndex,
            _ => {
                return None;
            }
        };
...
impl DataSource {
    pub fn unpack(data: &[u8], remaining: &mut usize) -> Option<Self> {
        match data[0] {
            ...
            1 => {
                *remaining -= 1;
                Some(DataSource::HostingLedgerObj)
            }
            2 => {
                *remaining -= 1;
                Some(DataSource::Header)
            }
            3 => {
                if *remaining < 65 {
                    return None;
                }
                let h: Hash = match data[1..65].try_into(){
                    Ok(h) => h,
                    Err(_e) => return None,
                };
                *remaining -= 65;
                Some(DataSource::CachedLedgerObj(h))
            }
            _ => {
                None
            }
...
impl LocatorUnpacker {
    pub fn unpack(data: Vec<u8>) -> Option<Self> {
        ...
        if data_total == 0 { return None; }
        let mut remaining = data_total;
        if let Some(data_source) = DataSource::unpack(&data, &mut remaining){
            let mut items = vec![];
            while remaining > 0 {
                if let Some(item) = LocatorPathItem::unpack(
                    &data[data_total - remaining..], &mut remaining) {
                    items.push(item);
                } else {
                    return None;
                }
...
#[derive(Debug)]
pub struct MockLedgerTransactionData {
    tx: serde_json::Value,
...
impl MockLedgerTransactionData {
    pub fn new(
        tx_str: &String,
        ...
        let cached_ledger_objs: HashMap<Hash, serde_json::Value> = HashMap::new();
        MockLedgerTransactionData { tx, hosting_ledger_obj, header, ledger, cached_ledger_objs, field_names }
    }

    pub fn read(&self, locators_buf: Vec<u8>) -> DataResult {
        if let Some(locator) = LocatorUnpacker::unpack(locators_buf) {
            let source = match locator.data_source {
                DataSource::Tx => 
                    &self.tx
                ,
                DataSource::HostingLedgerObj => &self.hosting_ledger_obj,
                DataSource::Header => &self.header,
                DataSource::CachedLedgerObj(hash) => {
                    if let Some(ledger_obj) = self.cached_ledger_objs.get(&hash) {
                        ledger_obj
                    } else {
                        return DataResult::LedgerObjectNotFound;
                    }
                }
            };
            
            let mut jv = source;
            for p in locator.items {
                jv = match p.item_type {
                    LocatorPathItemType::Sfield => {
                        ...
                        if let Some(sf_name) = sf_name {
                            ...
                            return DataResult::InvalidSfield;
                        }
                    }
                    LocatorPathItemType::ArrayIndex => {
                        if !jv.is_array() {
                            return DataResult::InvalidSfield;
                        }
                        let arr = jv.as_array().unwrap();
                        if p.value as usize >= arr.len() {
                            return DataResult::FieldNotFound;
                        }

                        arr.get(p.value as usize).unwrap()
                    }
                }
            }
            
            if jv.is_object() || jv.is_array() || jv.is_null(){
        ...
        DataResult::PathError
    }

    pub fn fetch_ledger_object(&mut self, key: Hash) -> DataResult {
        if let Some(ledger_obj) = self.ledger.get(&key) {
            self.cached_ledger_objs.insert(key, ledger_obj.clone());
            DataResult::FetchSuccess
        } else {
            DataResult::LedgerObjectNotFound
        }
    }
}
