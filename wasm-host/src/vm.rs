use crate::host_functions::{
    ...
    get_parent_ledger_time, get_tx_array_len, get_tx_field, 
    get_tx_nested_array_len, get_tx_nested_field,
...
// Recording functionality is now integrated directly into host functions
...
/// Run a WASM function with call recording
pub fn run_func_with_recording(
    wasm_file: String,
    func_name: &str,
    data_source: MockData,
    recorder: Rc<RefCell<CallRecorder>>,
) -> WasmEdgeResult<bool> {
    info!("Executing WASM function with recording: {}", func_name);
    let data_provider = DataProvider::new_with_recorder(data_source, recorder.clone());

    debug!("Setting up instance map and registering host functions with recording");
    let mut instances: HashMap<String, &mut dyn SyncInst> = HashMap::new();
    let mut import_builder = ImportObjectBuilder::new("host_lib", data_provider)?;

    // Recording versions of key functions
    info!("Linking `trace` function with recording");
    import_builder.with_func::<(i32, i32, i32, i32, i32), i32>("trace", trace)?;
