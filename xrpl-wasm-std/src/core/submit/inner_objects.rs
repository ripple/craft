...
fn encode_vl_length_inline(len: usize) -> (u8, u8, u8, u8) {
    if len <= 192 {
        ...
        (241 + (encoded >> 16) as u8, ((encoded >> 8) & 0xff) as u8, (encoded & 0xff) as u8, 3)
    } else {
        (0, 0, 0, 0)
    }
}
...
pub fn build_signer(
    ...
    buffer[pos] = b0;
    if len > 1 { buffer[pos + 1] = b1; }
    if len > 2 { buffer[pos + 2] = b2; }
    pos += len as usize;
...
pub fn build_array<const N: usize>(
    buffer: &mut [u8; N],
    objects: &[&[u8]]
) -> usize {
    let mut pos = 0;
