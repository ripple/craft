...
async function finishEscrow() {
  ...
  try {
    ...
    const mintTx = {
      ...
      Account: account,
      OracleDocumentID: 1,
      Provider: xrpl.convertStringToHex("test"),
      AssetClass: xrpl.convertStringToHex("crypto"),
    ...
    const mintResponse = await submit(mintTx, wallet)
    if (mintResponse.result.meta.TransactionResult === "tesSUCCESS") {
      console.log("\Oracle created successfully!")
    } else {
      console.error("\nFailed to finish escrow:", mintResponse.result.meta.TransactionResult)
    }
