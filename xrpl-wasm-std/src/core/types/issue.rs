...
use crate::core::types::uint_192::UINT192_SIZE;

/// XRP currency constant (all zeros)
pub const XRP_CURRENCY: CurrencyCode = CurrencyCode([0u8; 20]);
...
impl Issue {
    /// Create a new Issue
    pub const fn new(currency: CurrencyCode, account: AccountID) -> Self {
...
impl MPTIssue {
    ...
    pub fn from(sequence_num: u32, issuer: AccountID) -> Self {
        let mut bytes = [0u8; UINT192_SIZE];
        ...
        bytes[4..UINT192_SIZE].copy_from_slice(&issuer.0);
...
pub const fn is_xrp_currency(currency: CurrencyCode) -> bool {
    let mut i = 0;
    while i < 20 {
        if currency.0[i] != 0 {
            return false;
...
mod tests {
    ...
    fn test_is_xrp_currency() {
        ...
        assert!(!is_xrp_currency(CurrencyCode::from(*b"USD")));
    }
