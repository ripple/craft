...
pub fn list_fixtures() -> Result<()> {
    ...
    // Show fixtures in wasm-host-simulator directory (only for backward compatibility with escrow)
...
pub fn run_test(
    ...
    if !result.success {
        ...
        if let Some(desc) = result.error_description() {
            ...
            println!(
                "  • Check test fixtures in wasm-host-simulator/fixtures/<project>/{test_case}/"
            );
