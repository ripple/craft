...
pub extern "C" fn finish() -> i32 {
    ...
    let _ = trace_data("Testing 26 host functions (7 categories)", &[], DataRepr::AsHex);
...
fn test_transaction_data_functions() -> i32 {
    ...
    if fee_len != 8 {
        let _ = trace_num("ERROR: get_tx_field(Fee) wrong length (expected 8 bytes for XRP):", fee_len as i64);
    ...
    let _ = trace_data("Transaction Fee (serialized XRP amount):", &fee_buffer, DataRepr::AsHex);
...
fn test_current_ledger_object_functions() -> i32 {
    ...
    if balance_result <= 0 {
        ...
        let _ = trace_num("Current object balance length (XRP amount):", balance_result as i64);
        let _ = trace_data("Current object balance (serialized XRP amount):", &balance_buffer, DataRepr::AsHex);
    } else {
        let _ = trace_num("Current object balance length (non-XRP amount):", balance_result as i64);
...
fn test_any_ledger_object_functions() -> i32 {
    ...
    if cached_balance_result <= 0 {
        ...
        let _ = trace_num("Cached object balance length (XRP amount):", cached_balance_result as i64);
        let _ = trace_data("Cached object balance (serialized XRP amount):", &cached_balance_buffer, DataRepr::AsHex);
    } else {
        let _ = trace_num("Cached object balance length (non-XRP amount):", cached_balance_result as i64);
